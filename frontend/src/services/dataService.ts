...
export interface Notification {
	id: number
	message: string
	type: 'TASK' | 'SYSTEM' | 'DEADLINE' | 'MESSAGE' | string
	readFlag: boolean
	link?: string
	createdAt: string
	recipient?: any
...
export const userService = {
	...
	updateProfile: async (id: number, data: UserProfileUpdate): Promise<User> => {
		const response = await api.put<User>(`/users/${id}/profile`, data)
		return response.data
	},

	changePassword: async (id: number, data: PasswordChange): Promise<void> => {
		await api.put(`/users/${id}/password`, data)
	},

	updatePreferences: async (id: number, preferences: Record<string, any>): Promise<void> => {
		await api.put(`/users/${id}/preferences`, preferences)
	}
}
